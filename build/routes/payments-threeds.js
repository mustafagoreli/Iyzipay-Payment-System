...
var _default = router => {
  ...
  router.post("/threeds/payments/complete", async (req, res) => {
    ...
    if (req.body.status !== "success") {
      throw new _ApiError.default("Payment cant be starred because initilization is failed", 400, "initializationFailed");
    }
  ...
  router.post("/threeds/payments/:cartId/with-registered-card-token", _Session.default, async (req, res) => {
    ...
    if (!cardToken) {
      throw new _ApiError.default("Card token is required", 400, "cardTokanRequired");
    }
